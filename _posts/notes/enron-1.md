## Topic Modelling On Enron Email Dataset

This is a work in progress. Please consider it may contain some errors :)


* https://github.com/anthdm/ml-email-clustering
* http://www.cs.cmu.edu/~enron/
* https://medium.datadriveninvestor.com/nlp-with-lda-analyzing-topics-in-the-enron-email-dataset-20326b7ae36f
* https://medium.com/grabngoinfo/topic-modeling-with-deep-learning-using-python-bertopic-cf91f5676504
* https://medium.com/nlplanet/text-analysis-topic-modelling-with-spacy-gensim-4cd92ef06e06
* https://spacy.io/universe/project/bertopic
* https://www.kaggle.com/code/oybekeraliev/ernon-bert-lstm-model


## Enron Dataseti Hakkında
Enron Corpus 500.000'in üzerinde emailden oluşuyor. 158 adet çalışanın maillerini içeriyor. Şirket Aralık 2001 yılında mali bir çöküş yaşıyor. Soruşturma kapsamında kamuya açılan mailler daha sonra araştırmacılar tarafından kullanılmaya başlıyor.

Datanın yayınlanan versiyonu 423 MB tar ve gzip ile sıkıştırılmış. Sıkıştırılmamış versiyonu da 1.7Gb boyutunda. 

İnternet üzerinde farklı versiyonlarını bulmak mümkün. Örn. Kaggle versiyonu daha kullanışlı. file ve message olarak ayrılmış tek bir csv dosyasından oluşuyor.

Örn ilk satır <code>allen-p/_sent_mail/1.</code> ve <code>Message-ID: \<18782981.1075855378110.JavaMail.evans@thyme\> Date: Mon, 14 May 2001 16:39:00 -0700 (PDT...</code> gibi bir şekilde ayrılmış.
  
Burada daha detaylı bir şekilde text ve diğer verileri ayırmamız gerekiyor.
  
## Google Colab
* Bunun için bir google colab defteri açıyorum.
* Yardım kısmından dilini İngilizce yapalım. 

  <img src="https://raw.githubusercontent.com/csariyildiz/csariyildiz.github.io/main/img/goruntu1.png" class="img-fluid" alt="Enron Google Colab">

  
